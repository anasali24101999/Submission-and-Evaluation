...
if(form){const input=document.querySelector('#searchInput');const runSearch=value=>{const raw=value.trim();const q=raw.toLowerCase().replace(/beaches/g,'beach').replace(/temples/g,'temple');const filtered=destinations.filter(d=>`${d.name} ${d.country} ${d.type} ${d.description}`.toLowerCase().includes(q));render(q?filtered:destinations,raw);document.querySelector('#recommendations').scrollIntoView({behavior:'smooth'})};form.addEventListener('submit',e=>{e.preventDefault();runSearch(input.value)});document.querySelector('#clearBtn').addEventListener('click',()=>{input.value='';render(destinations);input.focus()});const initialQuery=new URLSearchParams(location.search).get('search');if(initialQuery){input.value=initialQuery;runSearch(initialQuery)}}
document.querySelectorAll('.header-search').forEach(searchForm=>searchForm.addEventListener('submit',e=>{e.preventDefault();const query=searchForm.querySelector('input').value.trim();if(query)location.href=`index.html?search=${encodeURIComponent(query)}#recommendations`}));
